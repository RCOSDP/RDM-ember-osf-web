...
export function validateRequiredIf(
    requiredIf: string, enabledIf_value: string, groups: SchemaBlockGroup[],
): ValidatorFunction {
    return async (
        key: string,
        newValue: string,
        ...
        if(enabledIf_value == ''){
            ...
            conditionMet = !newValue && !otherValue || (!newValue && otherValue === enabledIf_value);
